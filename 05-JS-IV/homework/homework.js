...
function eliminarPropiedad(objeto, unaPropiedad) {
    // Elimina la propiedad de objeto cuyo nombre está pasado por el 
    //parametro unaPropiedad 
...
function tienePropiedad(objeto, propiedad) {
    ...
    //tiene una propiedad (key) cuyo nombre es igual al valor 
...
function sumarLikesDeUsuario(usuario) {
    ...
    for (var i = 0; i < usuario.posts.length; i++) {
        totLikes += usuario.posts[i].likes;
    };
    return totLikes;
}

function agregarMetodoCalculoDescuento(producto) {
    // Agregar un método (función) al objeto "producto" 
    //llamado "calcularPrecioDescuento"
    ...
    // El método resta el descuento del precio y devuelve el precio con 
    //descuento
